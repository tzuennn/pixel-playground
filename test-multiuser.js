...
for (let i = 0; i < NUM_USERS; i++) {
  ...
  ws.on("message", (data) => {
    try {
      ...
      switch (msg.type) {
        ...
        case "pixel_updated":
          ...
          if (totalPixelsReceived <= 10) {
            console.log(
              `  👁️  User ${userId} saw: Pixel (${msg.x}, ${msg.y}) -> ${msg.color}`
            );
          }
          break;
      ...
      console.error(
        `❌ User ${userId} failed to parse message:`,
        error.message
      );
    }
  });
    ...
    setTimeout(() => {
      ...
      for (let p = 0; p < PIXELS_PER_USER; p++) {
        ...
        try {
          ...
          console.log(
            `  ✏️  User ${userId} drew pixel at (${x}, ${y}) in ${color}`
          );
        } catch (error) {
          console.error(
            `  ❌ User ${userId} failed to send pixel:`,
            error.message
          );
        }
      }
...
function showResults() {
  ...
  pixelsByUser.forEach((stats, userId) => {
    ...
    console.log(
      `  User ${userId}: Sent ${stats.sent}, Received ${stats.received} (${receivedFromOthers} from others)`
    );
  ...
  const successRate =
    (totalPixelsReceived / (totalPixelsSent * NUM_USERS)) * 100;
