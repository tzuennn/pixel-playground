...
async function getPodDistribution() {
  try {
    ...
    console.error(
      "⚠️  Could not fetch pod distribution (kubectl may not be available)"
    );
...
function verifyLoadBalancing() {
  ...
  const totalConnections = Array.from(stats.connectionsPerPod.values()).reduce(
    (sum, count) => sum + count,
    0
  );
  ...
  if (stats.connectionsPerPod.size > 0) {
    ...
    console.log(
      "⚠️  Could not verify pod distribution (requires kubectl access)"
    );
  }
}
...
function printStats() {
  ...
  const expectedBroadcasts =
    stats.clients.length * PIXELS_PER_CLIENT * stats.clients.length;
  const actualBroadcasts = stats.pixelUpdates;
  const effectiveness = ((actualBroadcasts / expectedBroadcasts) * 100).toFixed(
    1
  );
  ...
  if (reportedCounts.length > 0) {
    const avgReported =
      reportedCounts.reduce((a, b) => a + b, 0) / reportedCounts.length;
    ...
    if (accuracy > 90) {
      ...
      console.log(
        `   ⚠️  User count aggregation may have issues (${accuracy}%)`
      );
    }
  }
